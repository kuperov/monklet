...
class UnauthenticatedProfileTestCase(TestCase):
    ...
    def test_update_other_profile(self):
        ...
        edit_url = reverse_lazy(
            "users:profile-edit", kwargs={"pk": self.user.id}
        )  # self.user's profile page
        # regular user can't see others' update page
        self.assertContains(
            self.client.get(edit_url), "not authorized", status_code=403
        )
        payload = {"institution": "X", "location": "Y"}
        self.assertContains(
            self.client.post(edit_url, payload), "not authorized", status_code=403
        )
