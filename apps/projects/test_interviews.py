...
class InterviewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.bot_nc = models.Bot.objects.create(
            project=self.project, name="BBB", status="live", allow_public=True
        )
    ...
    def test_anonymous_no_letter(self):
        ...
        resp = self.client.post(
            landing_page,
            {"subject_name": "Bob", "subject_email": "", "proceed": "Proceed"},
            follow=False,
        )
    ...
    def test_invite(self):
        ...
        project_inv = reverse_lazy(
            "project-interviews-invite", kwargs={"pk": self.project.pk}
        )
        ...
        self.assertContains(
            resp, f'<option value="{self.bot_nc.pk}" selected>', status_code=200
        )
        # create invitation
        mail.outbox.clear()
        resp = self.client.post(
